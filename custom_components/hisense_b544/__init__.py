...
from .const import CONF_BAUDRATE, CONF_DEVICE, CONF_UNIT_ID, MESSAGE_SPACING, PLATFORMS
from .coordinator import HisenseB544Coordinator

type HisenseB544ConfigEntry = ConfigEntry[HisenseB544Coordinator]


def _params(entry: ConfigEntry) -> ModbusSerialParams:
...
async def async_setup_entry(hass: HomeAssistant, entry: HisenseB544ConfigEntry) -> bool:
    ...
    unit = async_get_unit(hass, entry, _params(entry), entry.data[CONF_UNIT_ID])
